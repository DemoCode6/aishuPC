...
export default {
  ...
  methods: {
    ...
    // 将用户添加到群组文档
    async groupdocAddUser2() {
      let that = this,
        toData = this.obj.nodes,
        userids = "",
        first_userDocId = false;
      try {
        for (const i in toData) {
          // 跳过包含children子节点的部门
          // 创建目录的接口
          if (!toData[i].children) {
            let userDocId = await this.addGroupDir(toData[i].label);
            let uid = toData[i].id; // 学生userid
            userids += "," + uid; // 学生userid集合
            // userids.push(uid)
            // 权限分享
            let params = {};
            ...
            // 权限分享接口
            await this.perm(params);
            params["docid"] = userDocId;
            params["inherit"] = false;
            await this.perm(params);
            // 给学生文件夹绑定学生id标签
            await this.bindTag(userDocId, uid);
            await this.bindTag(userDocId, that.value3.toString());
            // 记录第一个学生文件夹id
            if (!first_userDocId) {
              first_userDocId = userDocId;
            }
            that.$message({
              type: "success",
              message: "添加" + toData[i].label + "目录成功",
            });
          }
        }
        await this.bindTag(first_userDocId, userids.substr(1)); //给群组文档绑定学生userid集合
    ...
    async addGroupDir(name) {
      ...
      return res.data.docid; // 张三文件夹创建成功后的文件夹id
    },

    // 权限分享
    async perm(params) {
      await this.$axios.post("/api/v1/perm2/set", JSON.stringify(params)); // 分享群组文件夹到学生
    },

    // 绑定标签
    async bindTag(docid, tag) {
      let res = this.$axios.post(
        "/api/v1/file/addtag",
        JSON.stringify({
          docid,
          tag,
